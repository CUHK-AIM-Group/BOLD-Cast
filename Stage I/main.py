...
def get_args(model_name, dataset, custom_key="", yaml_path=None) -> argparse.Namespace:
    ...
    parser.add_argument('--test_lr', type=int, default=1e-3, help='test_lr')
    ...
    parser.add_argument('--save_root', type=str, default="./checkpoints", help='root for saving the model')
    parser.add_argument("--c_dim", default=100, help="Dimensionality of c", type=int)
    parser.add_argument("--p_dim", default=28, help="Dimensionality of p", type=int)
    parser.add_argument("--lr_max", default=1e-2, help="Learning rate for maximization", type=float)
    parser.add_argument("--lr_min", default=1e-6, help="Learning rate for minimization", type=float)
    ...
    parser.add_argument("--num_iters", default=100, help="Number of training iterations", type=int)
    parser.add_argument("--inner_epochs", default=10, help="Number of inner epochs", type=int)
    parser.add_argument("--phi_num_layers", default=3, help="Number of layers for phi", type=int)
    ...
    parser.add_argument("--decolayer", default=3, help="Number of decoder layers", type=int)
    parser.add_argument("--neighbor_num", default=190, help="Number of all sampled neighbor", type=int)
    parser.add_argument("--sample_neighbor", default=20, help="Number of sampled neighbor during each iteration", type=int)
    parser.add_argument("--sample_num", default=20, help="Number of sampled edges during each iteration", type=int)
    parser.add_argument("--tau", default=0.5, help="temperature in contrastive loss", type=int)
    parser.add_argument("--batch_size", default=4, help="temperature in contrastive loss", type=int)
...
def main():
    args = get_args(
        model_name="DMG",
        dataset="hcp", #acm imdb dblp freebase
