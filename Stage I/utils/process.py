...
def Dataset_Preprocess(root_path, data_path, num_rois=190, batch_size=4, window_size=80):
    """Legacy preprocessing helper retained for compatibility.

    New Stage-I training no longer uses this function. The hard-coded AAL-82
    dimensions and future-window target have been removed: dimensions are
    parameterized and the target is self-reconstruction (pre == his).
    """
    import deepdish as dd  # legacy-only dependency
    ...
    if timeseries.shape[0] != num_rois:
        raise ValueError(f'Expected {num_rois} ROIs, got {timeseries.shape[0]}')
    if adj.shape != (num_rois, num_rois):
        raise ValueError(f'Expected corr shape {(num_rois, num_rois)}, got {tuple(adj.shape)}')
    if timeseries.shape[1] < window_size:
        raise ValueError(f'Need at least {window_size} time points, got {timeseries.shape[1]}')

    a, _ = torch.topk(adj, min(20, num_rois), dim=1, largest=True, sorted=True)
    a_min = torch.min(a, dim=-1).values
    ge = torch.ge(adj, a_min.unsqueeze(-1))
    zero = torch.zeros_like(adj)
    adj = torch.where(ge, adj, zero)
    adj = adj.unsqueeze(0).repeat(batch_size, 1, 1)

    x = timeseries[:, :window_size].float()
    his = x.unsqueeze(0).repeat(batch_size, 1, 1)
    pre = his.clone()
    return adj, his, pre
